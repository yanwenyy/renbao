      ...
      >
        <el-form-item label="项目编号" prop="project.projectCode">
          <el-input
            @blur="
              verification(
                dataForm.project.projectCode,
                '项目编号不能重复',
                'projectCode'
              )
            "
            v-model="dataForm.project.projectCode"
            placeholder="项目编号"
          ></el-input>
        </el-form-item>
        <el-form-item label="项目名称" prop="project.projectName">
          <el-input
            @blur="
              verification(
                dataForm.project.projectName,
                '项目名称不能重复',
                'projectName'
              )
            "
            v-model="dataForm.project.projectName"
            placeholder="项目名称"
          ></el-input>
        </el-form-item>
        <el-form-item label="项目周期">
          ...
          <span>--</span>
          <el-form-item
            class="time-two time-two-nolabel"
            prop="project.projectPeriodEnd"
          >
            <el-date-picker
        ...
        <el-form-item label="被审核单位" prop="project.auditedUnit">
          <el-input
            v-model="dataForm.project.auditedUnit"
            placeholder="被审核单位"
          ></el-input>
        </el-form-item>
        <el-form-item label="项目成本">
          <el-button
            type="primary"
            @click="
              addTabel(dataForm.projectCosts, {
                projectItem: '',
                costMoney: '',
                costRemark: ''
              })
            ...
            <el-table-column width="50">
              <template slot-scope="scope">
                <i
                  ...
                  @click="removeRow(dataForm.projectCosts, scope.$index)"
                ></i>
              </template>
            </el-table-column>
            <el-table-column prop="name" label="成本事项">
              <template slot-scope="scope">
                <el-form-item
                  class="tabelForm"
                  :prop="'projectCosts.' + scope.$index + '.projectItem'"
                  :rules="{
                    required: true,
                    message: '成本事项不能为空',
                    trigger: 'blur'
                ...
                >
                  ...
                  <el-input
                    v-model="scope.row.projectItem"
                    placeholder="成本事项"
                  ></el-input>
                </el-form-item>
              </template>
            </el-table-column>
            <el-table-column prop="name" label="成本金额（元/天）">
              <template slot-scope="scope">
                <el-form-item
                  class="tabelForm"
                  :prop="'projectCosts.' + scope.$index + '.costMoney'"
        ...
        <el-form-item>
          ...
          >
            ...
            <el-table-column prop="name" label="小组组长" align="center">
              <template slot-scope="scope">
                <el-select
                  class="userList"
                  multiple
                  v-model="scope.row.leaderIds"
                  placeholder="请选择"
                >
                  <el-option
            ...
            <el-table-column prop="name" label="小组人员" align="center">
              <template slot-scope="scope">
                <el-select
                  class="userList"
                  multiple
                  v-model="scope.row.memberIds"
                  placeholder="请选择"
                >
                  <el-option
                    v-for="(item, index) in meberList"
        ...
        <el-form-item label="备注">
          <el-input
            ...
            type="textarea"
            :rows="4"
            v-model="dataForm.project.projectRemark"
            placeholder="备注"
          ></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="添加成员" :visible.sync="addMemberVisable">
      ...
      >
        ...
        <el-form-item label="选择人员">
          <el-input
            class="input-hasBtn"
            v-model="addMemberForm.dataAmount"
            placeholder="选择人员"
          ></el-input>
          <el-button type="primary" @click="memberListVisible = true"
            >选择</el-button
          >
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="添加小组成员" :visible.sync="addGroupMemberVisable">
      ...
      >
        <el-form-item label="小组名称">
          <el-input
            v-model="groupMemberForm.dataAmount"
            placeholder="小组名称"
          ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    verification(val, msg, name) {
      if (val) {
        if (val == this.projectData.project[name]) {
          return false;
        }
        this.$http({
    ...
    getRoleList(projectId) {
      ...
      this.$http({
        url: this.$http.adornUrl(
          projectId
            ? "/xmProjectGroup/selectPage"
            : "/xmProjectGroupTemplate/selectPage"
        ),
        method: "get",
        params: this.$http.adornParams({
          pageNo: 1,
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          this.$http({
            url: this.$http.adornUrl(
              `/xmProject/${
                !this.dataForm.project.projectId
                  ? "addProject"
                  : "updateProject"
              }`
            ),
