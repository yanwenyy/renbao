  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="排序号" prop="menuSort">
        <el-input-number v-model="dataForm.menuSort" controls-position="right" :min="0" label="排序号"></el-input-number>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      init (id) {
        ...
        this.$http({
          ...
          if (!this.dataForm.id) {
            ...
            // 修改
            this.$http({
              url: this.$http.adornUrl(`/menu/selectByUuid/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              var data=data.result;
              this.dataForm.id = data.menuId;
              this.dataForm.type = data.menuUrl?1:0;
              console.log(this.dataForm.type)
              this.dataForm.menuName = data.menuName;
              this.dataForm.menuParentId = data.menuParentId;
              this.dataForm.menuUrl = data.menuUrl;
              this.dataForm.menuSort = data.menuSort;
      ...
      menuListTreeCurrentChangeHandle (data, node) {
        this.dataForm.menuParentId = data.menuId
        this.dataForm.parentName = data.menuName;
        this.treeVisible=false;
      },
      // 菜单树设置当前选中节点
      menuListTreeSetCurrentNode () {
        this.$refs.menuListTree.setCurrentKey(this.dataForm.menuParentId)
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/menu/${!this.dataForm.id ? 'add' : 'updateByUuId'}`),
              method: 'post',
              data: this.$http.adornData({
                'menuId': this.dataForm.id || undefined,
                'menuName': this.dataForm.menuName,
                'menuParentId': this.dataForm.menuParentId,
